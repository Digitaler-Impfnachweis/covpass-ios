...
struct CertificateHolderMaskRequiredStatusViewModel: CertificateHolderImmunizationStatusViewModelProtocol {
    ...
    var description: String {
        guard let date = certificateHolderStatus.latestMaskRuleDate(for: userDefaults.stateSelection) else {
            return "infschg_detail_page_mask_mandatory_copy_1".localized
        }
        return String(format: "infschg_detail_page_mask_mandatory_copy_1".localized, DateUtils.displayDateFormatter.string(from: date))
    }
