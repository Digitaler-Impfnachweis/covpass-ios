...
public protocol CertificateReissueRepositoryProtocol {
    /// Renew a certificate based on an existing valid certificate. This operation verifies the
    ...
    /// wrong.
    func renew(_ webTokens: [ExtendedCBORWebToken]) -> Promise<CertificateReissueRepositoryResponse>
    /// Extend a  certificate based on an existing valid certificate. This operation verifies the
    /// authenticity and the integrity of the certificate, checks some formal rules of the content of the
    /// certificate and re-issues a new certificate with a different validity and/or re-encoded specifics
    /// (i.e. series encoding) than the original certificate.
    /// - Parameter webTokens: Array of  extended CBOR web tokens to renew.
    /// - Returns: A response object, if renewal succeeded. A `PromiseKit` error, if anything went
    /// wrong.
    func extend(_ webTokens: [ExtendedCBORWebToken]) -> Promise<CertificateReissueRepositoryResponse>
}
