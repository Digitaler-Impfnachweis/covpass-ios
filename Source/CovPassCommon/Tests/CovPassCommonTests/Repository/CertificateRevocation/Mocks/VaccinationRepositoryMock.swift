...
public class VaccinationRepositoryMock: VaccinationRepositoryProtocol {
    ...
    public func checkCertificate(_: String, expirationRuleIsActive: Bool, checkSealCertificate _: Bool) -> Promise<CBORWebToken> {
        Promise { seal in
            checkedCert != nil ? seal.fulfill(checkedCert!) : seal.reject(checkedCertError)
        }
    }

    public func validCertificate(_: String, expirationRuleIsActive: Bool, checkSealCertificate _: Bool) -> Promise<ExtendedCBORWebToken> {
        checkedCert != nil ?
            .value(
