...
extension View {
    ...
    ///   - topView: A view that can show up on top of currnet view - usualy contains an animation
    ///   - binding: Binding value that determines if the `topView` is visible or not
    /// - Returns: A new view that alters cuurent one by adding blurr and `topView` on top
    ...
    /// Use this method in order to show certain `LoadingView` on top of current view that is blurred our
    /// - Parameter binding: Binding value determinating if the view is visible or not
    /// - Returns: A new view that alters cuurent one by adding blurr and `LodaingView` on top
